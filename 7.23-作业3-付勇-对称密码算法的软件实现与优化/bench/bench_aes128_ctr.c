...
static sample_t run_sample(
    ...
    const uint8_t initial_counter[AES128_BLOCK_SIZE],
    const uint8_t *input,
    uint8_t *output,
    size_t bytes,
    size_t iterations
    ...
    for (size_t i = 0; i < iterations; ++i) {
        ...
        if (crypt(ctx, counter, input, output, bytes) != AES128_CTR_OK) {
            fputs("CTR benchmark operation failed\n", stderr);
            exit(EXIT_FAILURE);
        }
...
static result_t benchmark_one(
    ...
    memcpy(counter, initial_counter, sizeof(counter));
    if (crypt(ctx, counter, input, output, bytes) != AES128_CTR_OK) {
        fputs("CTR benchmark warm-up failed\n", stderr);
        exit(EXIT_FAILURE);
    }
    ...
    for (size_t i = 0; i < 7U; ++i) {
        samples[i] = run_sample(
            crypt, ctx, initial_counter, input, output, bytes, iterations
        );
...
int main(void) {
    ...
    printf("CTR AES-NI batch path: %s\n",
           aes128_ctr_aesni_supported() ? "enabled" : "software fallback");
    puts("AES-128-CTR reference vs T-table vs AVX2 vs AES-NI batch");
    ...
    for (size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]); ++s) {
        ...
        const result_t ref = benchmark_one(
            aes128_ctr_crypt_ref, &ctx, initial_counter,
            input, ref_output, bytes, iterations
        );
        const result_t table = benchmark_one(
            aes128_ctr_crypt_ttable, &ctx, initial_counter,
            input, table_output, bytes, iterations
        );
        const result_t avx = benchmark_one(
            aes128_ctr_crypt_avx2, &ctx, initial_counter,
            input, avx_output, bytes, iterations
        );
        const result_t ni = benchmark_one(
            aes128_ctr_crypt_aesni, &ctx, initial_counter,
            input, ni_output, bytes, iterations
        );
        ...
        if (memcmp(ref_output, table_output, bytes) != 0 ||
            ...
            fprintf(stderr, "CTR benchmark output mismatch at %zu bytes\n", bytes);
            free(input); free(ref_output); free(table_output); free(avx_output); free(ni_output);
            aes128_clear(&ctx);
            return EXIT_FAILURE;
        }

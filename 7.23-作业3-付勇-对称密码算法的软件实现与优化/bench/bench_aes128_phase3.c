...
int main(void) {
    ...
    printf("AVX2 hardware path: %s\n", aes128_avx2_supported() ? "enabled" : "unavailable (T-table fallback)");
    puts("AES-128 reference vs T-table vs AVX2 gather/shuffle");
    printf("%-9s %10s %10s %10s %9s %9s %9s %8s %8s\n",
           "Bytes", "Ref MiB/s", "T MiB/s", "AVX MiB/s",
           "Ref c/B", "T c/B", "AVX c/B", "T speed", "AVX speed");
    ...
    for (size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]); ++s) {
        ...
        if (input == NULL || ref_output == NULL || table_output == NULL || avx_output == NULL) {
            fprintf(stderr, "allocation failed for %zu bytes\n", bytes);
            free(input); free(ref_output); free(table_output); free(avx_output);
            aes128_clear(&ctx);
            return EXIT_FAILURE;
        }
        ...
        printf("%-9zu %10.2f %10.2f %10.2f %9.3f %9.3f %9.3f %7.2fx %7.2fx\n",
               bytes,
               ref.mib_per_second, table.mib_per_second, avx.mib_per_second,
               ref.cycles_per_byte, table.cycles_per_byte, avx.cycles_per_byte,

#include "aes128.h"

#include <string.h>

static const uint8_t sbox[256] = {
    
};

static const uint8_t inv_sbox[256] = {
    
};

static const uint8_t rcon[10] = {
    0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

static uint8_t xtime(uint8_t x) {
    return (uint8_t)(((uint16_t)x << 1) ^ ((uint16_t)(x >> 7) * UINT16_C(0x1b)));
}

static uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    for (unsigned i = 0; i < 8; ++i) {
        if ((b & 1U) != 0U) {
            result ^= a;
        }
        a = xtime(a);
        b >>= 1;
    }
    return result;
}

static void add_round_key(uint8_t state[16], const uint8_t *round_key) {
    for (size_t i = 0; i < 16; ++i) {
        state[i] ^= round_key[i];
    }
}

static void sub_bytes(uint8_t state[16]) {
    for (size_t i = 0; i < 16; ++i) {
        state[i] = sbox[state[i]];
    }
}

static void inv_sub_bytes(uint8_t state[16]) {
    for (size_t i = 0; i < 16; ++i) {
        state[i] = inv_sbox[state[i]];
    }
}

static void shift_rows(uint8_t state[16]) {
    uint8_t t;

    t = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = t;

    t = state[2];
    state[2] = state[10];
    state[10] = t;
    t = state[6];
    state[6] = state[14];
    state[14] = t;

    t = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = t;
}

static void inv_shift_rows(uint8_t state[16]) {
    uint8_t t;

    t = state[13];
    state[13] = state[9];
    state[9] = state[5];
    state[5] = state[1];
    state[1] = t;

    t = state[2];
    state[2] = state[10];
    state[10] = t;
    t = state[6];
    state[6] = state[14];
    state[14] = t;

    t = state[3];
    state[3] = state[7];
    state[7] = state[11];
    state[11] = state[15];
    state[15] = t;
}

static void mix_columns(uint8_t state[16]) {
    for (size_t c = 0; c < 4; ++c) {
        uint8_t *col = &state[4 * c];
        const uint8_t a0 = col[0];
        const uint8_t a1 = col[1];
        const uint8_t a2 = col[2];
        const uint8_t a3 = col[3];
        const uint8_t all = (uint8_t)(a0 ^ a1 ^ a2 ^ a3);

        col[0] ^= (uint8_t)(all ^ xtime((uint8_t)(a0 ^ a1)));
        col[1] ^= (uint8_t)(all ^ xtime((uint8_t)(a1 ^ a2)));
        col[2] ^= (uint8_t)(all ^ xtime((uint8_t)(a2 ^ a3)));
        col[3] ^= (uint8_t)(all ^ xtime((uint8_t)(a3 ^ a0)));
    }
}

static void inv_mix_columns(uint8_t state[16]) {
    for (size_t c = 0; c < 4; ++c) {
        uint8_t *col = &state[4 * c];
        const uint8_t a0 = col[0];
        const uint8_t a1 = col[1];
        const uint8_t a2 = col[2];
        const uint8_t a3 = col[3];

        col[0] = (uint8_t)(gf_mul(a0, 0x0e) ^ gf_mul(a1, 0x0b) ^ gf_mul(a2, 0x0d) ^ gf_mul(a3, 0x09));
        col[1] = (uint8_t)(gf_mul(a0, 0x09) ^ gf_mul(a1, 0x0e) ^ gf_mul(a2, 0x0b) ^ gf_mul(a3, 0x0d));
        col[2] = (uint8_t)(gf_mul(a0, 0x0d) ^ gf_mul(a1, 0x09) ^ gf_mul(a2, 0x0e) ^ gf_mul(a3, 0x0b));
        col[3] = (uint8_t)(gf_mul(a0, 0x0b) ^ gf_mul(a1, 0x0d) ^ gf_mul(a2, 0x09) ^ gf_mul(a3, 0x0e));
    }
}

int aes128_init(aes128_key_t *ctx, const uint8_t key[AES128_KEY_SIZE]) {
    if (ctx == NULL || key == NULL) {
        return -1;
    }

    memcpy(ctx->round_keys, key, AES128_KEY_SIZE);

    size_t generated = AES128_KEY_SIZE;
    unsigned rcon_index = 0;
    uint8_t temp[4];

    while (generated < AES128_EXPANDED_KEY_SIZE) {
        memcpy(temp, &ctx->round_keys[generated - 4], sizeof(temp));

        if ((generated % AES128_KEY_SIZE) == 0U) {
            const uint8_t first = temp[0];
            temp[0] = sbox[temp[1]];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[first];
            temp[0] ^= rcon[rcon_index++];
        }

        for (size_t i = 0; i < 4; ++i) {
            ctx->round_keys[generated] =
                (uint8_t)(ctx->round_keys[generated - AES128_KEY_SIZE] ^ temp[i]);
            ++generated;
        }
    }

    return 0;
}

void aes128_clear(aes128_key_t *ctx) {
    if (ctx == NULL) {
        return;
    }

    volatile uint8_t *p = ctx->round_keys;
    for (size_t i = 0; i < AES128_EXPANDED_KEY_SIZE; ++i) {
        p[i] = 0;
    }
}

void aes128_encrypt_block_ref(
    const aes128_key_t *ctx,
    const uint8_t in[AES128_BLOCK_SIZE],
    uint8_t out[AES128_BLOCK_SIZE]
) {
    uint8_t state[16];
    memcpy(state, in, sizeof(state));

    add_round_key(state, &ctx->round_keys[0]);

    for (size_t round = 1; round < AES128_ROUNDS; ++round) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, &ctx->round_keys[16 * round]);
    }

    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, &ctx->round_keys[16 * AES128_ROUNDS]);

    memcpy(out, state, sizeof(state));
}

void aes128_decrypt_block_ref(
    const aes128_key_t *ctx,
    const uint8_t in[AES128_BLOCK_SIZE],
    uint8_t out[AES128_BLOCK_SIZE]
) {
    uint8_t state[16];
    memcpy(state, in, sizeof(state));

    add_round_key(state, &ctx->round_keys[16 * AES128_ROUNDS]);

    for (size_t round = AES128_ROUNDS - 1; round > 0; --round) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, &ctx->round_keys[16 * round]);
        inv_mix_columns(state);
    }

    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, &ctx->round_keys[0]);

    memcpy(out, state, sizeof(state));
}

void aes128_encrypt_blocks_ref(
    const aes128_key_t *ctx,
    const uint8_t *in,
    uint8_t *out,
    size_t blocks
) {
    for (size_t i = 0; i < blocks; ++i) {
        aes128_encrypt_block_ref(ctx, in + 16 * i, out + 16 * i);
    }
}

void aes128_decrypt_blocks_ref(
    const aes128_key_t *ctx,
    const uint8_t *in,
    uint8_t *out,
    size_t blocks
) {
    for (size_t i = 0; i < blocks; ++i) {
        aes128_decrypt_block_ref(ctx, in + 16 * i, out + 16 * i);
    }
}
